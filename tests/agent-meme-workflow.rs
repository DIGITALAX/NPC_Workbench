...
mod tests {
    ...
    async fn test_load_nibble() {
        ...
        let ipfs_provider = IPFSProvider::Infura;
        let mut ipfs_config: HashMap<String, String> = HashMap::new();
        ipfs_config.insert("project_id".to_string(), env::var("PROJECT_ID").expect("PROJECT_ID must be set in .env file"));
        ipfs_config.insert("project_secret".to_string(), env::var("PROJECT_SECRET").expect("PROJECT_SECRET must be set in .env file"));
        let chain = Chain::PolygonAmoy;
        ...
        match new_nibble {
            Ok(mut new) => {
                ...
                match new
                    .load_nibble("0xe752d26460e31af2c34eccbca2a003da55bd42938eae052ca4fcad543fd98992")
                    .await
                {
                    Ok(nibble) => {
                        println!("Nibble loaded successfully {:?} \n\n {:?}", nibble.id, nibble.contracts);
                    }
                    Err(err) => {
                        println!("Error loading Nibble: {:?}", err)
                    }
                }
            }
            Err(err) => {
    ...
    // #[tokio::test]
    // async fn add_nibble_components() {
    //     dotenv().ok();
    //     let owner_private_key = env::var("PRIVATE_KEY").expect("PRIVATE_KEY must be set in .env file");
    //     let rpc_url = env::var("RPC").expect("RPC must be set in .env file");
    //     let ipfs_provider = IPFSProvider::Infura;
    ...
    //             match new
    //                 .load_nibble(vec![
    //                     61, 158, 209, 177, 196, 80, 226, 66, 229, 213, 66, 127, 94, 84, 181, 109,
    //                     75, 203, 9, 144, 70, 121, 227, 116, 162, 133, 12, 32, 158, 45, 212, 30,
    //                 ])
    //                 .await
    //             {
    //                 Ok(mut nibble) => {
